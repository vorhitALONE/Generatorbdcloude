...
app.post("/api/generate", (req, res) => {
  ...
  if (pending.length === 0) {
    return res.status(400).json({ ok: false, error: sequence.length === 0 ? "Список пуст" : "Все пользователи использованы" });
  }
...
app.get("/api/sequence/stats", (req, res) => {
  res.json({ ok: true, total: sequence.length, used: sequence.filter(x => x.used).length, remaining: sequence.filter(x => !x.used).length });
});
...
app.post("/api/admin/sequence/add", requireAuth, (req, res) => {
  ...
  for (const v of numbers) {
    ...
    if (!val) continue;
    if (sequence.some(x => x.value === val && !x.used)) continue;
    sequence.push({ id: Date.now() + Math.random(), value: val, used: false });
    added++;
  }
